...
class WepayService
{
    ...
    public function preApprovalCreate($accessToken, $donation, $accountId)
    {
        if (is_null($accessToken)) {
            $accessToken = $this->accessToken;
        }
        ...
        $wepay = new WePay($accessToken);
        // $wepay = new WePay('STAGE_73179b0539182e798c5aff8363548743285eceb6a6ebfe9ac596d48384f1369b');
        // $accountId = '1911836184';
        ...
        // foreach ($donations as $donation) {
            try{
                $response = $wepay->request('credit_card/authorize', array(
                    ...
                    // 'amount'            => $donation->amount,
                    // 'currency'          => 'USD',
                    // 'short_description' => 'MyRespects authorization',
                    // 'type'              => 'goods',
                    // 'payment_method'    => array(
                    //     'type'        => 'credit_card',
                    //     'credit_card' => array(
