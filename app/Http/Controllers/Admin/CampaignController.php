...
class CampaignController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($filter) {
            if ($request->get('filter') === 'campaigns') {
                $items = Campaign::whereNotNull('user_id')->orderBy($order, $sort)->paginate(20);
            }
        }else {
            $items = Campaign::orderBy($order, $sort)->paginate(20);
        }

        // $items = Campaign::paginate(15);
        
        return view('admin.campaign.index', compact('items', 'filter', 'order', 'sort'));
    }
    ...
    public function update(Request $request, Campaign $campaign)
    {
        ...
        $campaign->featured = $data['featured'];
